...
/** ul 태그로 구성된 컴포넌트 정의 --> styled.태그 이름``; (역따옴표 주의) */
...
const StyledComponent = () => {
  ...
  return (
    <div>
      <Meta title="StyledComponent" description="styled-component" />
      <h1>StyledComponent</h1>

      <h3>단순 태그처럼 사용</h3>
      <MyGridContainer>
        {/* MyGridItm에게 width라는 이름의 변수값 전달 */}
        <MyGridItem width={"30%"}>
          <MyBox>Item1</MyBox>
        </MyGridItem>
        <MyGridItem width={"10%"}>
        ...
        {myColors.map((item, index) => {
          return (
            <MyGridItem key={index} width={myWidth}>
              {/* StyledComponent로 전달하는 props는 `$`를 이름 앞에 명시 */}
