  ...
  const onDataEditSubmit = useCallback(
    (e) => {
      ...
      try {
        ...
        regexHelper.value('#edit-form input[name="loc"]', "학과위치를 입력하세요.");
        regexHelper.maxLength('#edit-form input[name="loc"]', 20, "학과위치는 20자 이내로 입력하세요.");
      } catch (e) {
        alert(e.message);
        e.element.focus();
        return;
      ...
      (async () => {
        ...
        console.log(`수정할 데이터의 인덱스 : ${editId}`);
            ...
            {department.map((v, i) => {
              if (updateId === v.id) {
                return (
                  <tr key={`edit-${v.id}`}>
                    <td>
                      {/* `value`속성은 반드시 onChange 이벤트와 함께 사용해야함 */}
                      {/* onChange 없이 사용할 경우 `defaultValue` 속성을 사용 */}
